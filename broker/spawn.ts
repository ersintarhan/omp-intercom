...
// Under Bun (omp's runtime, or `bun test`) the broker runs TypeScript natively,
// so no tsx CLI is needed. Under Node (pi) keep the node + tsx launch.
function isBunRuntime(runtimePath: string): boolean {
  const executableName = runtimePath.split(/[\\/]/).pop();
  return Boolean(executableName && /^bun(?:\.exe)?$/i.test(executableName));
}

function getDefaultBrokerArgv(brokerPath: string, extensionDir: string, runtimePath: string): { command: string; args: string[] } {
  if (isBunRuntime(runtimePath)) {
    return { command: runtimePath, args: [brokerPath] };
  }
  return { command: getNodeCommand(runtimePath), args: [getTsxCliPath(extensionDir), brokerPath] };
}

// Test helper: full argv for launching the broker with the current runtime
// (bun runs broker.ts directly; node goes through the bundled tsx CLI).
export function getBrokerProcessArgv(brokerPath: string, extensionDir: string = EXTENSION_DIR): [string, ...string[]] {
  const { command, args } = getDefaultBrokerArgv(brokerPath, extensionDir, process.execPath);
  return [command, ...args];
}
...
export function getWindowsBrokerCommandLine(
  ...
  if (usesDefaultBrokerCommand(brokerCommand, brokerArgs)) {
    const { command, args } = getDefaultBrokerArgv(brokerPath, extensionDir, nodePath);
