...
export const INTERCOM_PROTOCOL_NAME = "omp-intercom";
...
export function getAgentDirPath(
  ...
  // Prefer $HOME/$USERPROFILE over os.homedir(): Bun resolves homedir() from
  // the passwd entry at process start and ignores later process.env changes,
  // which breaks tests and env-redirected runs.
  homeDir: string = env.HOME?.trim() || env.USERPROFILE?.trim() || homedir(),
  cwd: string = process.cwd(),
): string {
  // omp itself honors PI_CODING_AGENT_DIR (default ~/.omp/agent); follow the host.
...
export function getBrokerSocketPath(
  ...
  if (platform === "win32") {
    return `\\\\.\\pipe\\omp-intercom-${sanitizePipeSegment(agentDir)}`;
  }
