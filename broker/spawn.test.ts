...
test("spawnBrokerIfNeeded includes stderr from default broker startup failures", async () => {
  ...
  // Under Bun the default broker launch runs broker.ts natively; under Node it
  // goes through the bundled tsx CLI. Fail whichever entrypoint will run.
  ...
  try {
    ...
    if (runningUnderBun) {
      writeFileSync(path.join(brokerDir, "broker.ts"), "process.stderr.write('fake tsx failed\\n'); process.exit(1);\n");
    }
...
test("isBrokerHealthOkMessage requires the intercom protocol marker", () => {
  assert.equal(isBrokerHealthOkMessage({ type: "health_ok", requestId: "req-1", protocol: "omp-intercom", version: 1 }, "req-1"), true);
  assert.equal(isBrokerHealthOkMessage({ type: "health_ok", requestId: "req-1" }, "req-1"), false);
  assert.equal(isBrokerHealthOkMessage({ type: "health_ok", requestId: "req-2", protocol: "omp-intercom", version: 1 }, "req-1"), false);
