...
test("getAgentDirPath defaults to the omp agent directory under home", () => {
  assert.equal(getAgentDirPath({}, "/home/rcroh"), join("/home/rcroh", ".omp/agent"));
});

test("getAgentDirPath honors PI_CODING_AGENT_DIR", () => {
  assert.equal(getAgentDirPath({ PI_CODING_AGENT_DIR: "/tmp/omp-agent" }, "/home/rcroh"), "/tmp/omp-agent");
});
...
test("getBrokerSocketPath uses named pipe on Windows", () => {
  const pipePath = getBrokerSocketPath("win32", "C:/Users/rcroh/.omp/agent");
  assert.match(pipePath, /^\\\\\.\\pipe\\omp-intercom-/);
...
test("getBrokerSocketPath uses broker.sock under PI_CODING_AGENT_DIR on non-Windows", () => {
  const socketPath = getBrokerSocketPath("linux", "/tmp/omp-agent");
  assert.equal(socketPath, join("/tmp/omp-agent", "intercom", "broker.sock"));
});
...
test.skipIf(process.platform === "win32")("ensureIntercomRuntimeDir creates and repairs restrictive Unix directory permissions", () => {
  const root = mkdtempSync(join(tmpdir(), "pi-intercom-paths-"));
  const intercomDir = join(root, "intercom");
...
test.skipIf(process.platform === "win32")("restrictIntercomRuntimeFile applies restrictive Unix file permissions", () => {
  const root = mkdtempSync(join(tmpdir(), "pi-intercom-paths-"));
