...
import { getBrokerProcessArgv } from "./spawn.ts";
...
async function startBroker(agentDir: string): Promise<ChildProcessWithoutNullStreams> {
  const [brokerCommand, ...brokerArgs] = getBrokerProcessArgv(path.join(repoDir, "broker", "broker.ts"));
  const broker = spawn(
    brokerCommand,
    brokerArgs,
    {
...
test("extension bus negotiates, routes, elects an owner, and persists state", async () => {
...
test("extension bus owners, publish, and state are scoped", async () => {
