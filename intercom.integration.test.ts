...
async function withNamePollMs<T>(namePollMs: string, fn: () => T | Promise<T>): Promise<T> {
  const previous = process.env.PI_INTERCOM_NAME_POLL_MS;
  process.env.PI_INTERCOM_NAME_POLL_MS = namePollMs;
  try {
    return await fn();
  } finally {
    if (previous === undefined) delete process.env.PI_INTERCOM_NAME_POLL_MS;
    else process.env.PI_INTERCOM_NAME_POLL_MS = previous;
  }
}
...
test("opt-in TCP broker requires endpoint state for health and registration", async () => {
  const net = await import("node:net");
  const { readFileSync } = await import("node:fs");
  const { createMessageReader, writeMessage } = await import("./broker/framing.ts");
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-intercom-tcp-agent-"));
  const brokerArgv = getBrokerProcessArgv(path.join(repoDir, "broker", "broker.ts"));
  const broker = spawn(brokerArgv[0], [
    ...brokerArgv.slice(1, -1),
    "-e",
...
async function setupClients() {
  const brokerArgv = getBrokerProcessArgv(path.join(repoDir, "broker", "broker.ts"));
  const broker = spawn(brokerArgv[0], brokerArgv.slice(1), {
    cwd: repoDir,
...
function pendingAskRecordPath(messageId: string): string {
  return path.join(sharedHomeDir, ".omp", "agent", "intercom", "pending-asks", `${encodeURIComponent(messageId)}.json`);
}
...
test("broker accepts caller supplied stable IDs across reconnect", async () => {
...
test("broker scopes discovery, routing, mailbox, and presence", async () => {
  const brokerArgv = getBrokerProcessArgv(path.join(repoDir, "broker", "broker.ts"));
  const broker = spawn(brokerArgv[0], brokerArgv.slice(1), {
    cwd: repoDir,
    env: { ...process.env, HOME: sharedHomeDir, USERPROFILE: sharedHomeDir },
...
test("broker rotates endpoint epochs and replays same message ids without duplicate delivery", async () => {
...
test("delivery records keep colon-containing sender and message IDs distinct", async () => {
...
test("client re-resolves a rebound exact target once with the same message id", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const replacement = new IntercomClient();
...
test("broker rejects malformed exact target fields instead of falling back to name routing", async () => {
  const { orchestrator, cleanup } = await setupClients();
...
test("broker rejects changed message content after a rebound exact-target failure", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker propagates a session's tmux pane id into the roster", async () => {
  const { planner, cleanup } = await setupClients();
  const worker = new IntercomClient();
...
test("broker omits tmux pane id for sessions outside tmux", async () => {
  const { planner, cleanup } = await setupClients();
  const worker = new IntercomClient();
...
test("broker owns local trust metadata instead of trusting registration payloads", async () => {
  const { planner, cleanup } = await setupClients();
...
test("broker rejects unknown replyTo values instead of delivering forged replies", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker disconnects a connection that exceeds the local rate limit", async () => {
...
test("broker idle raw connections cannot block legitimate registration", async () => {
  const net = await import("node:net");
  const { getBrokerSocketPath } = await import("./broker/paths.ts");
  const { cleanup } = await setupClients();
...
test("broker times out sockets that unregister and go idle", async () => {
  const { cleanup } = await setupClients();
...
test("unnamed sessions use a collision-resistant runtime alias", async () => {
  ...
  try {
    ...
    assert.equal(first.name, "omp-chat-019fe418-248e-7447");
    assert.equal(second.name, "omp-chat-019fe418-248e-7abc");
...
test("broker coalesces no-op presence floods", async () => {
  const { planner, cleanup } = await setupClients();
  const worker = new IntercomClient();
...
test("old stable-ID socket cannot mutate the replacement session", async () => {
  const { planner, cleanup } = await setupClients();
...
test("stable-ID replacement preserves old ask edges and ignores stale cancels", async () => {
  const { orchestrator, cleanup } = await setupClients();
...
test("broker resolves unique short IDs and rejects ambiguous prefixes", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const first = new IntercomClient();
  const second = new IntercomClient();
...
test("intercom tool prefers exact names over ID prefixes", async () => {
  const { orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
  const evilPrefix = new IntercomClient();
...
test("extension can pin a restart-stable intercom session id", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("intercom-id inserts a stable handoff snippet into the editor", async () => {
  const { cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
  ...
  try {
    ...
    assert.match(editorText, /Existing note\n\nUse omp-intercom: intercom\(\{ action: "send", to: "session-child-test", message: "\.\.\." \}\)/);
...
test("alias names the current session, opens the local input menu, and appears in intercom displays", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
  ...
  try {
    ...
    await withNamePollMs("60000", async () => {
      piIntercomExtension(harness.pi as never);
      await harness.emitLifecycle("session_start");
...
test("alias reports no-UI usage and current alias without hanging", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
  const output: string[] = [];
...
test("intercom tool shows unique ID prefixes when names collide", async () => {
  const { cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("late extension registration advertises before an onReady publish", async () => {
  const { cleanup } = await setupClients();
...
test("extension outbox sends notify-only messages with trace and provenance", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("extension outbox rejects duplicate request ids without duplicate delivery", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("obsolete toolVisibility config never hides or reveals the intercom tool", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  ...
  await withIntercomConfig({ toolVisibility: "after-first-use" }, async () => {
    ...
    try {
      ...
      await inboundHarness.emitLifecycle("tool_result", {
        toolName: "read",
        input: { path: path.join(repoDir, "skills", "omp-intercom", "SKILL.md") },
        isError: false,
      });
      assert.equal(inboundHarness.getActiveTools().includes("intercom"), true);
...
test("sessions publish automatic lifecycle status", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  ...
  try {
    ...
    // omp has no model_select event; presence picks up the model at turn_start.
    const freshEventContext = {
      ...harness.ctx,
      model: { id: "fresh-model" },
    };
    await harness.emitLifecycle("turn_start", {}, freshEventContext);
...
test("idle name poll propagates /name changes without other activity", async () => {
  const { planner, cleanup } = await setupClients();
  let sessionName = "idle-name-before";
  const harness = createExtensionHarness(() => sessionName, { hasUI: true });

  try {
    await withNamePollMs("25", async () => {
      const { default: piIntercomExtension } = await import("./index.ts");
      piIntercomExtension(harness.pi as never);
      await harness.emitLifecycle("session_start");
...
test("turn_start re-registers when Pi replaces the session context", async () => {
  const { planner, cleanup } = await setupClients();
...
test("busy interactive sessions steer top-level asks without aborting", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let abortCount = 0;
  let idle = false;
...
test("idle interactive sessions trigger a new turn immediately", async () => {
	const { default: piIntercomExtension } = await import("./index.ts");
	const { planner, cleanup } = await setupClients();
...
test("broker rejects changed duplicate message IDs and replays identical sends without reinjection", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
...
test("busy interactive sessions steer same-sender messages in sequence order", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("explicit cancel acknowledges that a steered inbound message may already be processed", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("intercom cancel action requests cancellation for a sent message", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("same-sender supersede reports an already-steered inbound message", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("supersede is scoped to the same sender and receiver", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, orchestrator, cleanup } = await setupClients();
  const idle = false;
...
test("replied steered asks are not injected again after the current turn", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("deferred startup connect is cancelled on shutdown", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
...
test("stale overlay work stops after same-session restart", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let customCalls = 0;
...
test("steered inbound messages are not reinjected after shutdown", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let idle = false;
...
test("busy non-interactive sessions auto-reply to top-level asks without aborting", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
  let abortCount = 0;
...
test("regular intercom asks fail safely when started concurrently", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { orchestrator, cleanup } = await setupClients();
...
test("broker writes a local pending ask record for delivered blocking asks", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker removes a pending ask record after a delivered reply", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker removes a pending ask record after asker cancellation", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker removes a pending ask record during timeout pruning", async () => {
...
test("broker refuses reverse mutual asks until the original ask is answered", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("a reply can start a new reverse ask", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("failed replies do not clear broker mutual-ask edges", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("regular intercom ask timeout reports message id and delivery state", async () => {
  const previousTimeout = process.env.PI_INTERCOM_ASK_TIMEOUT_MS;
  process.env.PI_INTERCOM_ASK_TIMEOUT_MS = "50";
  const { default: piIntercomExtension } = await import("./index.ts");
  const { planner, cleanup } = await setupClients();
...
test("regular intercom ask cancellation clears broker mutual-ask edge", async () => {
  const { default: piIntercomExtension } = await import("./index.ts");
  const { orchestrator, cleanup } = await setupClients();
...
test("full ask/reply round-trip works with reply target resolved from current turn context", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("intercom reply targets exact replyTo when multiple asks are pending", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("intercom reply sends attachments", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("intercom send refuses a different target during an active inbound ask turn", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("intercom reply targets one of multiple pending asks by short session ID", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("a short-ID reply unblocks the original ask when another ask is pending", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("broker queues replies to recently disconnected named senders", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const replacement = new IntercomClient();
...
test("broker rejects blocking asks to disconnected targets", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker never remaps a disconnected mailbox back to the sending session", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker does not treat runtime fallback aliases as reconnect identities", async () => {
  const { orchestrator, cleanup } = await setupClients();
  const original = new IntercomClient();
  const unrelated = new IntercomClient();
  const replacement = new IntercomClient();
  const fallbackAlias = "omp-chat-019fe418-248e-7447";
...
test("broker does not deliver explicit mailbox mail to a matching fallback alias", async () => {
  const { orchestrator, cleanup } = await setupClients();
  const original = new IntercomClient();
  const fallback = new IntercomClient();
  const replacement = new IntercomClient();
  const sharedName = "omp-chat-shared-worker";
...
test("broker preserves mailbox reconnects for explicit omp-chat names", async () => {
  const { orchestrator, cleanup } = await setupClients();
  const original = new IntercomClient();
  const replacement = new IntercomClient();
  const explicitName = "omp-chat-explicit-worker";
  const originalId = "explicit-omp-chat-original";
  ...
  try {
    ...
    assert.equal((await orchestrator.send(originalId, {
      messageId: "explicit-omp-chat-mail",
      text: "Explicit names keep mailbox reconnect semantics.",
    })).delivered, true);
    const [, message] = await delivered;
    assert.equal(message.id, "explicit-omp-chat-mail");
  } finally {
    await original.disconnect().catch(() => undefined);
    await replacement.disconnect().catch(() => undefined);
    await cleanup();
...
test("broker delivers old-id replies to an already reconnected same-name sender", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const replacement = new IntercomClient();
...
test("broker keeps queued mail away from a same-name session in another cwd", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
...
test("broker delivers queued mail to a relaunch reporting the same cwd differently", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const replacement = new IntercomClient();
...
test("broker does not reroute an id-addressed message to a same-name session in another cwd", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const otherProject = new IntercomClient();
...
test("intercom reply queues mail for a disconnected named sender", async () => {
  const { planner, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("async ask can be replied to later from the single pending ask fallback", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const replyTracker = new ReplyTracker();
...
test("presence carries context usage to peers, and an explicit null clears a stale value", async () => {
...
test("public send infers a reply from the sole pending ask, resolves the waiter, and dismisses it", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("an intentional unrelated public notification during a sole pending ask is still inferred as its reply", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("confirmSend still gates an inferred reply; cancellation preserves the pending ask", async () => {
  await withConfirmSendEnabled(async () => {
    const { planner, orchestrator, cleanup } = await setupClients();
    const { default: piIntercomExtension } = await import("./index.ts");
...
test("caller-supplied replyTo takes precedence over inference and skips the inferred result text", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("multiple pending asks from the same sender leave a send unthreaded and preserve every ask", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("an expired pending ask is not inferred; the send remains ordinary", async () => {
...
test("intercom ask fails fast when the target is not currently connected", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("send falls back to an exact stored ID or name for a disconnected asker but never guesses from an ID prefix", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
  const { default: piIntercomExtension } = await import("./index.ts");
...
test("failed delivery from an inferred reply preserves the pending ask", async () => {
  const { planner, orchestrator, cleanup } = await setupClients();
